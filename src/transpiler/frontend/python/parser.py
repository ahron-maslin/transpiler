...
class PythonFrontend:
    ...
    def _parse_type(self, node: ast.expr) -> Type:
        if isinstance(node, ast.Name):
            if node.id == "int": return IntType
            elif node.id == "float": return FloatType
            elif node.id == "bool": return BoolType
            elif node.id == "str": return StringType
        return AnyType
    ...
    def _visit_expr(self, node: ast.expr) -> Expr:
        if isinstance(node, ast.Constant):
            if isinstance(node.value, bool): return BoolLiteral(value=node.value)
            elif isinstance(node.value, int): return IntLiteral(value=node.value)
            elif isinstance(node.value, float): return FloatLiteral(value=node.value)
            elif isinstance(node.value, str): return StringLiteral(value=node.value)
            elif node.value is None: return NullLiteral()
        elif isinstance(node, ast.Name):
            return Var(name=node.id)
        elif isinstance(node, ast.BinOp):
            op = None
            if isinstance(node.op, ast.Add): op = BinOp.ADD
            elif isinstance(node.op, ast.Sub): op = BinOp.SUB
            elif isinstance(node.op, ast.Mult): op = BinOp.MUL
            elif isinstance(node.op, ast.Div): op = BinOp.DIV
            elif isinstance(node.op, ast.Mod): op = BinOp.MOD
            left = self._visit_expr(node.left)
            right = self._visit_expr(node.right)
            if op: return Binary(op=op, left=left, right=right)
        elif isinstance(node, ast.Compare):
            if len(node.ops) == 1 and len(node.comparators) == 1:
                op = None
                if isinstance(node.ops[0], ast.Eq): op = BinOp.EQ
                elif isinstance(node.ops[0], ast.NotEq): op = BinOp.NEQ
                elif isinstance(node.ops[0], ast.Lt): op = BinOp.LT
                elif isinstance(node.ops[0], ast.LtE): op = BinOp.LTE
                elif isinstance(node.ops[0], ast.Gt): op = BinOp.GT
                elif isinstance(node.ops[0], ast.GtE): op = BinOp.GTE
                left = self._visit_expr(node.left)
