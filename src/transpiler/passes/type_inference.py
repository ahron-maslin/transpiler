...
class TypeInferencePass:
    ...
    def _infer_stmt(self, node: Stmt):
        if isinstance(node, Let):
            ...
            if node.type.kind == TypeKind.ANY and inferred_type and inferred_type.kind != TypeKind.ANY:
    ...
    def _infer_expr_type(self, node: Expr) -> Type:
        ...
        elif isinstance(node, FloatLiteral): return FloatType
        elif isinstance(node, BoolLiteral): return BoolType
        elif isinstance(node, StringLiteral): return StringType
        elif isinstance(node, NullLiteral): return AnyType
        elif isinstance(node, Var):
            return self.env.get(node.name, AnyType)
        elif isinstance(node, Binary):
            if node.op in [BinOp.EQ, BinOp.NEQ, BinOp.LT, BinOp.LTE, BinOp.GT, BinOp.GTE, BinOp.AND, BinOp.OR]:
                return BoolType
            left_type = self._infer_expr_type(node.left)
            right_type = self._infer_expr_type(node.right)
            if left_type.kind != TypeKind.ANY: return left_type
            if right_type.kind != TypeKind.ANY: return right_type
            return AnyType
