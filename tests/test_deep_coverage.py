...
class TestCLILanguageDispatch:
    ...
    @pytest.mark.parametrize("ext,content,to_lang", [
    ...
    @pytest.mark.parametrize("to_lang,expected_substr", [
        ("go", "func"),
        ("rust", "fn"),
        ("java", "public"),
        ("c", "int"),
...
class TestCFrontendControlFlow:
    ...
    def test_all_comparison_ops_parsed(self):
        ...
        for op_c, op_ir in [("==", BinOp.EQ), ("!=", BinOp.NEQ), ("<", BinOp.LT),
    ...
    def test_all_arithmetic_ops_parsed(self):
        for op_c, op_ir in [("+", BinOp.ADD), ("-", BinOp.SUB), ("*", BinOp.MUL),
                            ("/", BinOp.DIV), ("%", BinOp.MOD)]:
            prog = self.fe.parse(f"""
            int f(int a, int b) {{
                return a {op_c} b;
...
class TestRustFrontendControlFlow:
    ...
    def test_all_comparison_ops(self):
        for op_rs, op_ir in [("==", BinOp.EQ), ("!=", BinOp.NEQ), ("<", BinOp.LT),
                             ("<=", BinOp.LTE), (">", BinOp.GT), (">=", BinOp.GTE)]:
            prog = self.fe.parse(f"""
...
class TestTypeInferenceDeep:
        ...
        prog = self._run(Program(functions=[
            Function("f", [], VoidType, Block([
                For(
                    ...
                    update=Assign(Var("i"), Binary(BinOp.ADD, Var("i"), IntLiteral(1))),
    ...
    def test_right_side_type_fallback(self):
        ...
        prog = self._run(Program(functions=[
            Function("f", [], VoidType, Block([
                Let("x", AnyType, Binary(BinOp.ADD, NullLiteral(), IntLiteral(5)))
            ]))
        ]))
    ...
    def test_field_access_stays_any(self):
        prog = self._run(Program(functions=[
            Function("f", [], VoidType, Block([
                Let("x", AnyType, FieldAccess(Var("obj"), "field"))
            ]))
        ]))
    ...
    def test_unhandled_expr_stays_any(self):
        ...
        prog = self._run(Program(functions=[
            Function("f", [], VoidType, Block([
                Let("x", AnyType, ArrayInit([IntLiteral(1)]))
            ]))
        ]))
        assert prog.functions[0].body.statements[0].type.kind == TypeKind.ANY
