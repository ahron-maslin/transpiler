...
class PythonBackend:
    ...
    def _generate_stmt(self, node: Stmt) -> str:
        if isinstance(node, Return):
            ...
            return f"{node.name}: {self._type_to_str(node.type)} = {self._generate_expr(node.value)}"
    ...
    def _type_to_str(self, t: Type) -> str:
        ...
        elif t.kind == TypeKind.FLOAT: return "float"
        elif t.kind == TypeKind.BOOL: return "bool"
        elif t.kind == TypeKind.STRING: return "str"
        return "Any"
