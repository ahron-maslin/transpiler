...
class RustBackend:
    ...
    def _generate_expr(self, node: Expr) -> str:
        ...
        elif isinstance(node, StringLiteral): return f'"{node.value}".to_string()'
        elif isinstance(node, NullLiteral): return "None"
    ...
    def _type_to_str(self, t: Type) -> str:
        if t.kind == TypeKind.INT: return "i32"
        elif t.kind == TypeKind.FLOAT: return "f64"
        elif t.kind == TypeKind.BOOL: return "bool"
        elif t.kind == TypeKind.STRING: return "String"
        elif t.kind == TypeKind.VOID: return "()"
